...
func (s *server) getFlightInfo(_ context.Context, key string, _ int64) (*flight.FlightInfo, error) {
	s3fs := s3iofs.NewWithClient(s.bucket, s.s3Client)

	s3file, err := s3fs.Open(key)
	if err != nil {
		return nil, err
	}

	pr, err := file.NewParquetReader(s3file.(parquet.ReaderAtSeeker))
	if err != nil {
		return nil, err
	}
	defer pr.Close()
...
func (s *server) DoGet(tkt *flight.Ticket, fs flight.FlightService_DoGetServer) error {
	...
	sf, err := s3fs.Open(path)
	if err != nil {
		return err
	}

	pr, err := file.NewParquetReader(sf.(parquet.ReaderAtSeeker))
	if err != nil {
		return err
	}
	defer pr.Close()
